...
async def search_books(q: str = Query(..., description="검색어"), ddc: Optional[str] = Query(None, description="십진분류법 기호(선택 사항)")):
  """
  read365 외부 API로 실시간 검색
  예: /api/books?q=코스모스&ddc=4
  """
  return await library_api_service.search_books(keyword=q, ddc=ddc)
